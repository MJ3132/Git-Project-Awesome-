...
router.get("/collab/all/:gigsterId?", function (req, res) {

    console.log(req.params.gigsterId);


    Collaborator.find({ userId :req.params.gigsterId})
    .populate('projectId')
    .exec()
    .then((collaborator) => {

        console.log("found collaborations " + collaborator);

    //    let collaborationProjects = collaborator.map(collaboration =>  collaboration.projectId)
...
router.post("/collab-pending", function (req, res) {
    ...
    console.log("collab body " + req.body);
